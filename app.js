  ...
  addButtons(slots, (selected) => {
    ...
    setTimeout(() => {
      ...
      addButtons(['✅ Подтвердить запись'], () => {
        ...
        // Здесь можно отправить данные в бота через tg.sendData
        const data = {
          category: currentCategoryKey,
          service: currentService.name,
          master: selectedMaster || 'не выбран',
          time: selectedTime
        };
        // Отправляем данные в бот (если бот ожидает)
        // tg.sendData(JSON.stringify(data)); // раскомментировать при наличии бэкенда
        setTimeout(() => {
...
// ---------- КОНТАКТ ----------
function contactMe() {
  // Открываем чат с твоим ботом (используем BOT_LINK из data.js)
